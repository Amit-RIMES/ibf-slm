...
def _index_exists(conn, index: str) -> bool:
    row = conn.execute(
        sa.text("SELECT name FROM sqlite_master WHERE type='index' AND name=:i"),
        {"i": index},
    ).fetchone()
    return row is not None
...
def upgrade() -> None:
    ...
    if not _col_exists(conn, "forecast_uploads", "variable"):
        op.add_column(
            "forecast_uploads",
    ...
    if not _col_exists(conn, "ecmwf_config", "parameters"):
        op.add_column(
            "ecmwf_config",
    ...
    if not _table_exists(conn, "cds_config"):
        op.create_table(
            "cds_config",
    ...
    if not _table_exists(conn, "glofas_records"):
        op.create_table(
            "glofas_records",
            sa.Column("id", sa.Integer(), primary_key=True),
    ...
    if not _index_exists(conn, "ix_glofas_records_forecast_date"):
